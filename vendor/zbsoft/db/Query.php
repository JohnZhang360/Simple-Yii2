...
class Query extends Object
{
     ...
     * $query = (new \zbsoft\db\Query)->from(['u' => 'user', 'profile']);
     *
     * // SELECT * FROM (SELECT * FROM `user` WHERE `active` = 1) `activeusers`;
     * $subquery = (new \zbsoft\db\Query)->from('user')->where(['active' => true])
     * $query = (new \zbsoft\db\Query)->from(['activeusers' => $subquery]);
     *
