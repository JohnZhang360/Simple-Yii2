...
use zbsoft\exception\NotSupportedException;
 ...
 *
 * ```php
 * $connection = new \zbsoft\db\Connection([
 *     'dsn' => $dsn,
 *     'username' => $username,
 *     'password' => $password,
 * ]);
...
class Connection extends Object
{
     ...
     */
    public $schemaMap = [
        'mysqli' => 'zbsoft\db\mysql\Schema', // MySQL
        'mysql' => 'zbsoft\db\mysql\Schema', // MySQL
        'sqlite' => 'zbsoft\db\sqlite\Schema', // sqlite 3
        'sqlite2' => 'zbsoft\db\sqlite\Schema', // sqlite 2
        'sqlsrv' => 'zbsoft\db\mssql\Schema', // newer MSSQL driver on MS Windows hosts
        'oci' => 'zbsoft\db\oci\Schema', // Oracle driver
        'mssql' => 'zbsoft\db\mssql\Schema', // older MSSQL driver on MS Windows hosts
        'dblib' => 'zbsoft\db\mssql\Schema', // dblib drivers on GNU/Linux (and maybe other OSes) hosts
    ];
    /**
     * @var string Custom PDO wrapper class. If not set, it will use [[PDO]] or [[zbsoft\db\mssql\PDO]] when MSSQL is used.
     * @see pdo
     */
    public $pdoClass;
    /**
     ...
     * @since 2.0.7
     */
    public $commandClass = 'zbsoft\db\Command';
    /**
     ...
     * Use 0 to indicate that the cached data will never expire.
     * @param \zbsoft\caching\Dependency $dependency the cache dependency associated with the cached query results.
     ...
     * @param \zbsoft\caching\Dependency $dependency the preferred caching dependency. If null, it will be ignored.
    ...
    public function getQueryCacheInfo($duration, $dependency)
    {
        ...
        if ($duration === 0 || $duration > 0) {
            if (is_string($this->queryCache) && Zb::$app) {
                $cache = Zb::$app->get($this->queryCache, false);
            } else {
    ...
    protected function createPdoInstance()
    {
        ...
        if ($pdoClass === null) {
            ...
            if (isset($driver)) {
                if ($driver === 'mssql' || $driver === 'dblib') {
                    $pdoClass = 'zbsoft\db\mssql\PDO';
                } elseif ($driver === 'sqlsrv') {
                    $pdoClass = 'zbsoft\db\mssql\SqlsrvPDO';
                }
            }
        }
        ...
        if (strncmp('sqlite:@', $dsn, 8) === 0) {
            $dsn = 'sqlite:' . Zb::getAlias(substr($dsn, 7));
        }
    ...
    public function getSchema()
    {
        if ($this->_schema !== null) {
            ...
            if (isset($this->schemaMap[$driver])) {
                ...
                return $this->_schema = Zb::createObject($config);
            } else {
    ...
    protected function openFromPool(array $pool, array $sharedConfig)
    {
        ...
        $cache = is_string($this->serverStatusCache) ? Zb::$app->get($this->serverStatusCache, false) : $this->serverStatusCache;
        ...
        foreach ($pool as $config) {
            ...
            /* @var $db Connection */
            $db = Zb::createObject($config);

            try {
                $db->open();
                return $db;
            } catch (\Exception $e) {
                Zb::warning("Connection ({$config['dsn']}) failed: " . $e->getMessage(), __METHOD__);
                if ($cache instanceof Cache) {
