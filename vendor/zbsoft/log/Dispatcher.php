...
namespace zbsoft\log;

use Zb;
use zbsoft\base\ErrorHandler;
...
class Dispatcher extends Object
{
    ...
    public function init()
    {
        ...
        foreach ($this->targets as $name => $target) {
            if (!$target instanceof Target) {
                $this->targets[$name] = Zb::createObject($target);
            }
        }
    }
     ...
     * If not set, [[\Zb::getLogger()]] will be used.
     * @property Logger the logger. If not set, [[\Zb::getLogger()]] will be used.
     * @return Logger the logger.
     */
    public function getLogger()
    {
        if ($this->_logger === null) {
            $this->setLogger(Zb::getLogger());
        }
        return $this->_logger;
    }
     ...
     * or a configuration that will be used to create one using [[Zb::createObject()]].
     */
    public function setLogger($value)
    {
        if (is_string($value) || is_array($value)) {
            $value = Zb::createObject($value);
        }
        $this->_logger = $value;
