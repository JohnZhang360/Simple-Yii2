 ...
 *
 * Session is a Web application component that can be accessed via `Zb::$app->session`.
 *
...
class Session extends Object implements \IteratorAggregate, \ArrayAccess, \Countable
{
    ...
    protected function registerSessionHandler()
    {
        if ($this->handler !== null) {
            if (!is_object($this->handler)) {
                $this->handler = Zb::createObject($this->handler);
            }
     ...
     * ```php
     * <?php
     * foreach (Zb::$app->session->getAllFlashes() as $key => $message) {
